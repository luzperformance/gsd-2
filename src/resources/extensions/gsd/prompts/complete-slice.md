...
5. If verification fails:
   - For task-specific regressions introduced by a completed task, call `gsd_task_reopen` with that task and a concrete reason so execution can redo the work. Determine "task-specific" by comparing before/after verification evidence, checking whether the failure is localized to that task's changed files or artifacts, attempting reproduction with and without that task's outputs, and reviewing relevant logs or test diffs.
   - For inherited/out-of-scope failures (issues that preexisted this slice or affect project-wide baselines, such as existing CI baseline failures or upstream infra regressions), do **not** reopen completed tasks. Heuristic: if failure timestamp < `slice.start` or impact is mostly outside slice-owned files, call `gsd_replan_slice` with the blocker and adjusted verification scope or follow-up verification tasks.
   - For other plan-invalidating failures (failures that break this slice's execution assumptions, such as API contract changes or missing dependencies discovered during the slice), call `gsd_replan_slice` with the blocker and updated execution tasks. Heuristic: if the failure prevents current tasks from completing or changes required execution steps, treat it as plan-invalidating.
